...
import { getSuspiciousTransactionList, setSuspiciousTransactionListError } from '../../../store/slices/transaction-managment/suspicious-transaction-monitoring/suspiciousTransactionListSlice';
...
export default function SuspiciousTransactionMonitoring() {
  ...
  const { loading, error, success, suspiciousTransactionList, deleteSuccess } = useSelector((state) => state.suspiciousTransactionList);

  useEffect(() => {
    setSelectedProducts(null);
    dispatch(setSuspiciousTransactionListError(null));
/*
...
*/
  ...
  const columns = [
    { field: 'field1', header: t('transaction.suspiciousTransactionColumn1'), sortable: true, className: "primary-text center-column", },
    
    { 
      field: 'field2', 
      header: t('transaction.suspiciousTransactionColumn2'),
      sortable: true, 
      body: (rowData) => (
          rowData.field2 === 'İade' ? 
    ...
    { field: 'field3', header: t('transaction.suspiciousTransactionColumn3'), className: "price-column center-column", sortable: true, },
    { field: 'field4', header: t('transaction.suspiciousTransactionColumn4'), sortable: true, },
    { field: 'field5', header: t('transaction.suspiciousTransactionColumn5'), sortable: true, },

    { 
      field: 'field6', 
      header: t('transaction.suspiciousTransactionColumn6'),
      sortable: true, 
      body: (rowData) => (
...

    {
        ...
        body: (rowData) => (
            <div className="row-user-buttons">
                ...
                <Link to={`/detail-suspicious-transaction/${rowData.guid}`}>
  ...
  const handleFilter = (filters) => {
    let filtered = [...suspiciousTransactionList];
  ...
  const handleSelect = (code) => {
    if(code == 'csv') exportDataToCSV(suspiciousTransactionList, columns, selectOptions, 'suspiciousTransactions');;
    if(code == 'xlsx') exportDataToExcel(suspiciousTransactionList, columns, selectOptions, 'suspiciousTransactions');
  ...
  const header = (
    <div className='d-flex align-items-center'>
      <div className='flex-fill'>
        <div className="title">{t("transaction.suspiciousTransactionTitle")}</div>
        <div className="text">{t("transaction.suspiciousTransactionText")}</div>
      </div>
  ...
  useEffect(() => {
    if (selectedDateFilter) {
      ...
      dispatch(getSuspiciousTransactionList({
    ...
    <>
      ...
      <div className="datatable-area-container">
        { !loading ? (
          <DataTable 
            header={header}
            value={filteredList || suspiciousTransactionList} 
            paginator 
            rows={10} 
