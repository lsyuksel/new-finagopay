...
export const showDialog = (type, message, icon, acceptCallback) => {
    const dialogConfig = {
        group: 'ConfirmDialogTemplating',
        message: (
            <div className={`dialog-flex-box ${type === 'success' ? 'success-color' : 'danger-color'}`}>
                <i><img src={icon} alt="" /></i>
                <span>{message.title}</span>
                <div dangerouslySetInnerHTML={{ __html: message.content }} />
            </div>
        ),
        acceptClassName: type === 'success' ? 'success-color' : 'danger-color',
